...
function App() {
  ...
  const getData = () => {
    ...
    for (let i = 0; i < apiData.length; i++) {
      ...
      arr[0] +=
        apiData[i].actualsTimeseries[0].cases == null
          ? 0
          : apiData[i].actualsTimeseries[0].cases;
      for (let j = 1; j < apiData[i].actualsTimeseries.length; j++) {
        if (arr[j] === undefined) arr[j] = 0;
        arr[j] +=
          apiData[i].actualsTimeseries[j].cases == null
            ? 0
            : apiData[i].actualsTimeseries[j].cases -
              apiData[i].actualsTimeseries[j - 1].cases;
      }
    }
  ...
  const getDeathData = () => {
    ...
    for (let i = 0; i < apiData.length; i++) {
      if (arr[0] === undefined) arr[0] = 0;
      arr[0] +=
        apiData[i].actualsTimeseries[0].deaths == null
          ? 0
          : apiData[i].actualsTimeseries[0].deaths;
      for (let j = 1; j < apiData[i].actualsTimeseries.length; j++) {
        if (arr[j] === undefined) arr[j] = 0;
        arr[j] +=
          apiData[i].actualsTimeseries[j].deaths == null
            ? 0
            : apiData[i].actualsTimeseries[j].deaths -
              apiData[i].actualsTimeseries[j - 1].deaths;
      }
    }
    var arr1 = Array(Math.floor(arr.length / 7)).fill(0);
    let j = 0;
  ...
  const getVaccineData = () => {
    ...
    for (let i = 0; i < apiData.length; i++) {
      if (arr[0] === undefined) arr[0] = 0;
      arr[0] +=
        apiData[i].actualsTimeseries[0].vaccinationsCompleted == null
          ? 0
          : apiData[i].actualsTimeseries[0].vaccinationsCompleted;
      for (let j = 1; j < apiData[i].actualsTimeseries.length; j++) {
        if (arr[j] === undefined) arr[j] = 0;
        arr[j] +=
          apiData[i].actualsTimeseries[j].vaccinationsCompleted == null
            ? 0
            : apiData[i].actualsTimeseries[j].vaccinationsCompleted -
              apiData[i].actualsTimeseries[j - 1].vaccinationsCompleted;
      }
    }
    var arr1 = Array(Math.floor(arr.length / 7)).fill(0);
    let j = 0;
  ...
  useEffect(() => {
    if (date.length > 0) setLoading(false);
  }, [date]);
  ...
  return (
    <div class="App">
      ...
      <div class="header">JPs Covid Tracker</div>
      <div class="cells">
        <div class="first-col">
          <div class="cell">
            <div>
              ...
              {apiData?.map((el, i) => {
                return (
                  <div class="data">
                    ...
                    <div class="cases">
                      CASES: {el.actuals.newCases} | DEATHS:{" "}
                      {el.actuals.newDeaths}
                    </div>
                    <div class="vaccines">
                      TOTAL VACCINES: {el.actuals.vaccinationsCompleted}
                    </div>
                    <hr />
                  </div>
                );
              ...
              {loading && (
                <div class="loadings">
                  <div class="img-container">
                    <img src={loadingIcon} alt="" class="img" />
                  </div>
                </div>
              )}
              {!loading && (
